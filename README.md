...
Result (`bench/history.json`): 88 commits changed at least one function, none was skipped, and 2 of those 88 changed a function its own tests never ran, 4 of 301 changed functions in total. Both belong to upstream PR #419 and flag the same two functions: c327d6c adds `_read_jsonl_file` and `_read_csv_file`, and b47acc1 moves them to `tabulate/cli.py`. The maintainer's next commit, 791c4bf, adds tests for the new CLI options, and 17bf1cf runs them in-process; with the suite at 17bf1cf both functions run (checked with the same script). So on a well-tested project Witnessed raised nothing on the other 86 commits and fired on a single pull request, where the gap was real and the maintainer closed it within the same pull request. This counts alarms; it is not a precision or recall figure, because the history has no ground truth for gaps that nobody noticed.
